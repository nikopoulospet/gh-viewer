...
def main():
    ...
    with open(APP_FILE, "w") as f:
        json.dump({"client_id": app["client_id"], "slug": app["slug"],
                   "name": app["name"], "html_url": app["html_url"],
                   "owner": args.org or "user", "settings_url": settings_url}, f, indent=2)

    # The toggles live on the App's SETTINGS page, which is under the owning
    # org when --org was used - not the public /apps/<slug> page.
    settings_url = (
        f"https://github.com/organizations/{args.org}/settings/apps/{app['slug']}"
        if args.org else f"https://github.com/settings/apps/{app['slug']}"
    )

    print(f"\n[ok] registered: {app['html_url']}")
    print(f"[ok] owner: {'org ' + args.org if args.org else 'your user account'}")
